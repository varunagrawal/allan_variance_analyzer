    ...
    def run(self):
        ...
        for i in tqdm(range(int(self.sequence_time_ * self.update_rate_))):
            ...
            accelerometer_bias += RandomNormalDistributionVector(
                key, self.accelerometer_random_walk_) * np.sqrt(dt)
            gyroscope_bias += RandomNormalDistributionVector(
                key, self.gyroscope_random_walk_) * np.sqrt(dt)

            acc_measure = accelerometer_real + accelerometer_bias + RandomNormalDistributionVector(
                key, self.accelerometer_noise_density_) / np.sqrt(dt)
            gyro_measure = gyroscope_real + gyroscope_bias + RandomNormalDistributionVector(
                key, self.gyroscope_noise_density_) / np.sqrt(dt)
