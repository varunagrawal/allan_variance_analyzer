...
def get_intercept(x, y, m, b):
    ...
    coeffs, _ = curve_fit(line_func,
                          logx,
                          logy,
                          bounds=([m, -np.inf], [m + 0.001, np.inf]))
...
def generate_prediction(tau,
                        q_quantization=0,
                        q_white=0,
                        q_bias_instability=0,
                        q_walk=0,
                        q_ramp=0):
    """
    ...
    params = np.array([
        q_quantization**2, q_white**2, q_bias_instability**2, q_walk**2,
        q_ramp**2
    ])
...
def compute_white_noise_params(period, measurements, white_noise_break_point):
    ...
    for idx, _ in enumerate("xyz"):
        wn_intercept[idx], wn_fit_fn[idx] = get_intercept(
            ...
            measurements[0:white_noise_break_point, idx], -0.5, 1.0)
...
def plot_loglog(period: np.ndarray,
    ...
    for idx, c in enumerate("rgb"):
        ...
        plt.loglog(period[measurement_min_index[idx]],
                   measurement_min[idx],
                   f"{c}^",
                   markersize=20)
    ...
    fig.savefig(f'{measurement_type.lower()}.png',
                dpi=600,
                bbox_inches="tight")
...
def accelerometer_analysis(period, acceleration, white_noise_break_point):
    ...
    accel_rr_intercept, accel_fit_rr = compute_rate_random_walk(
        period, acceleration)
    ...
    for idx, axis in enumerate("XYZ"):
        print(f"{axis} Bias Instability: {accel_min[idx]: .5f} m/s^2",
              f"{accel_min[idx]*3600*3600: .5f} m/hr^2")

    for idx, axis in enumerate("XYZ"):
        print(
            f"{axis} Accel Random Walk: {accel_rr_intercept[idx]: .5f} m/s^2/sqrt(s)"
        )
...
def gyroscope_analysis(period, rotation_rate, white_noise_break_point):
    ...
    # Compute rate random walk
    # gradient=0.5, intercept at t=3
    gyro_rr_intercept, gyro_fit_rr = compute_rate_random_walk(
        period, rotation_rate)
    ...
    for idx, axis in enumerate("XYZ"):
        print(
            f"{axis} Rate Random Walk: {gyro_rr_intercept[idx]: .5f} deg/s/sqrt(s)"
        )
...
def write_imu_yaml(worst_accel_white_noise: float,
                   worst_accel_random_walk: float,
                   worst_gyro_white_noise: float,
                   worst_gyro_random_walk: float, update_rate: int):
    """
    ...
    with open("imu.yaml", "w") as yaml_file:
        ...
        yaml_file.write(
            f"accelerometer_noise_density: {worst_accel_white_noise}\n")
        yaml_file.write(
            f"accelerometer_random_walk: {worst_accel_random_walk}\n")
        yaml_file.write("\n")
        ...
        yaml_file.write(
            f"update_rate: {update_rate} #Make sure this is correct\n")
