...
class TestAnalysis(unittest.TestCase):
    ...
    def setUp(self):
        data = np.loadtxt(
            current_dir / "fixtures" / "allan_variance.csv",
            delimiter=" ",
            dtype=float,
    ...
    def test_compute_white_noise_params(self):
        ...
        np.testing.assert_allclose(
            expected_accel_white_noise_params, accel_wn_intercept
        )
        ...
        # regression
        np.testing.assert_allclose(expected_gyro_white_noise_params, gyro_wn_intercept)
    ...
    def test_compute_rate_random_walk(self):
        ...
        accel_rr_intercept_x, xfit_rr = get_intercept(
            self.period, self.acceleration[:, 0], 0.5, 3.0
        )
        accel_rr_intercept_y, yfit_rr = get_intercept(
            self.period, self.acceleration[:, 1], 0.5, 3.0
        )
        accel_rr_intercept_z, zfit_rr = get_intercept(
            self.period, self.acceleration[:, 2], 0.5, 3.0
        )
    ...
    def test_compute_bias_instability(self):
        ...
        np.testing.assert_allclose(
            np.asarray([accel_min_x_index, accel_min_y_index, accel_min_z_index]),
            accel_argmin,
        )
    ...
    def test_accelerometer_analysis(self):
        ...
        # regression
        self.assertAlmostEqual(0.0025615284313200383, worst_accel_white_noise, 11)
        self.assertAlmostEqual(9.079971439106243e-05, worst_accel_random_walk, 11)
    ...
    def test_gyroscope_analysis(self):
        ...
        # regression
        self.assertAlmostEqual(0.010936249927095876, worst_gyro_white_noise, 11)
        self.assertAlmostEqual(0.0002359578060906847, worst_gyro_random_walk, 11)
